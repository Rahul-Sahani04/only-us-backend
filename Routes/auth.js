...
// **URL:** `/createuser`  
// **Method:** `POST`  
// **Description:** Creates a new user with the provided email, password, and username.
...
// **Response:**
// - Success: `{ "success": true, "authToken": "token" }`
// - Error: `{ "errors": "Login with correct credentials" }`
...
// **Method:** `PUT`  
// **Description:** Updates the online status of the authenticated user.
...
// **URL:** `/findFreeUser`  
// **Method:** `POST`  
// **Description:** Finds a free user who is online and not currently connected.

// **Response:**
// - Success: `{ ...userDetails }`
// - Error: `{ "message": "Internal server error", "usersAvailables": false }`
...
// **URL:** `/updateDisconnect`  
// **Method:** `PUT`  
// **Description:** Updates the status of a user to offline based on the provided socket ID.
...
// **Response:**
// - Success: `{ "message": "Disconnected", "success": "true" }`
// - Error: `{ "message": "Error while updating offline status" }`
...
// **URL:** `/makeFriend`  
// **Method:** `POST`  
// **Description:** Makes two users friends based on their socket IDs.

// **Request Body:**
// ```json
// {
//   "current_connection": "socket123"
// }
// ```

// **Response:**
// - Success: `{ "message": "You are now friends... Enjoy", "success": true, "friendTimerExpires": timestamp }`
// - Error: `{ "error": "Error making friends" }`
...
router.post("/makeFriend", fetchuser, async (req, res) => {
  ...
  if (!secondUsersocketId) {
    return res.status(400).json({ message: "current_connection is required" });
  }
  ...
  try {
    ...
    if (firstUser.friends.includes(secondUser._id) || secondUser.friends.includes(firstUser._id)) {
      return res.status(400).json({ message: "Friend already exists" });
    }

    const updatedFirstUser = await User.findByIdAndUpdate(userId, { $push: { friends: secondUser._id } }, { new: true });
    const updatedSecondUser = await User.findByIdAndUpdate(secondUser._id, { $push: { friends: firstUser._id } }, { new: true });

    if (!updatedFirstUser || !updatedSecondUser) {
      return res.status(500).json({ message: "Error updating friends list" });
    }

    return res.status(200).json({ message: "You are now friends... Enjoy", success: true });
  } catch (error) {
    console.error(error);
...
// **Response:**
// - Success: `{ "friendsCount": count, "friendUsernames": [ ... ] }`
// - Error: `{ "message": "Internal server error" }`
...
// ? Change Password
// **URL:** `/changePass/:id/:token`
// **Method:** `PUT`
// **Description:** Changes the password of the user with the provided ID and token.
...
// **URL:** `/reactionUpdate`
// **Method:** `PUT`
// **Description:** Updates the status of a user to free based on the provided socket ID.
...
// **Response:**
// - Success: `{ "message": "Updated Successfully", "success": true }`
